...
function doGet(e) {
  ...
  template.isSuper = perms.users;      // User Access panel
  template.isAdmin = perms.admin;      // Admin Panel
  template.canEdit = perms.entries;    // Add Stock / Packing / Supervisor buttons
  template.canEditRows = perms.edit;   // Edit modals
  template.canDelete = perms.delete;
  template.userRole = role;
  const out = template.evaluate()
    .setTitle("Meethi Golee Inventory")
    .addMetaTag("viewport", "width=device-width, initial-scale=1");
  // Optional browser-tab / shortcut icon: a public .png or .ico URL (see CONFIG.FAVICON_URL)
  if (CONFIG.FAVICON_URL) { try { out.setFaviconUrl(CONFIG.FAVICON_URL); } catch (e) { /* ignore bad URL */ } }
  return out;
}
...
function addRawMaterialReceived(productName, qtyMl) {
  ...
  qtyMl = Number(qtyMl);
  if (!qtyMl || qtyMl <= 0) throw new Error("Invalid quantity.");
...
function addPackagingReceived(itemId, qtyPcs) {
  ...
  if (!qtyPcs || qtyPcs <= 0) throw new Error("Invalid quantity.");
  const t = readTable_(CONFIG.SHEETS.PACK);
...
function recordPacking(productName, qtyPacked, opts) {
  ...
  if (!qtyPacked || qtyPacked <= 0) throw new Error("Invalid quantity.");
  opts = opts || {};
...
function createSupEntry_(email, entryType, productName, qty, remarks, channel, orderId, repackItems, saleValue, batchNo, extra) {
  ...
  else if (saleValue !== "" && saleValue !== null && saleValue !== undefined) { sale = Number(saleValue); if (isNaN(sale) || sale < 0) throw new Error("Invalid sale value."); }
...
function updateSupervisorEntry(entryId, entryType, productName, qty, remarks, channel, orderId, repackItems, saleValue, batchNo, extra) {
  ...
  else { const sv = Number(saleValue); if (isNaN(sv) || sv < 0) throw new Error("Invalid sale value."); s[cSale] = sv; }
...
function updateRates(changes) {
  ...
  function setField(itemId, colIdx, label, value) {
    ...
    value = Number(value);
    if (isNaN(value) || value < 0) throw new Error("Invalid price for " + itemId);
    const i = idx[itemId];
  ...
  if (chChanges.length) {
    ...
    chChanges.forEach(function (c) {
      ...
      if (val !== "" && (isNaN(val) || val < 0)) throw new Error("Invalid channel price for " + c.product + " / " + c.channel);
...
function checkLoginPasscode(enteredEmail, passcode) {
  const actualEmail = getCurrentUserEmail_();
  if (String(enteredEmail).trim().toLowerCase() !== String(actualEmail).trim().toLowerCase()) {
    throw new Error("That email doesn't match your logged-in Google account (" + actualEmail + ").");
  }

  const sh = getSheet_(CONFIG.SHEETS.ADMIN);
  const lastRow = sh.getLastRow();
  if (lastRow < 2) throw new Error("Access not configured yet. Contact your admin.");

  const data = sh.getRange(2, 1, lastRow - 1, 4).getValues();
  const key = String(actualEmail).trim().toLowerCase();
  for (let i = 0; i < data.length; i++) {
    if (String(data[i][0]).trim().toLowerCase() === key) {
      ...
      if (storedPasscode !== String(passcode).trim()) throw new Error("Incorrect passcode.");
      const acc = getUserAccess_(actualEmail);
